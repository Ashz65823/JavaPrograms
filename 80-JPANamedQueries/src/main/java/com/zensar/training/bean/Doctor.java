...
@Entity
@Table(name="Doctors100")
//we can write multiple names query element which is put in array
@NamedQueries(
		{
		...
		@NamedQuery(name="deleteFile",query="delete Doctor d  where d.id=:deleteId")
		})

@NamedNativeQueries({
	@NamedNativeQuery(name="readAll",query = "select * from doctors100",resultClass = Doctor.class),
	@NamedNativeQuery(name="readById",query = "select * from doctors100 where id=?",resultClass = Doctor.class)
})
public class Doctor {
